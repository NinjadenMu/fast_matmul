...
Although I implement optimizations for the Apple M4, the techniques used in this repo hold across basically all modern CPU architectures (for example, porting over to x86 basically just requires tuning some cache/register block sizes and swapping NEON intrinsics for AVX.)  In fact, my project [MuBLIS](https://github.com/NinjadenMu/MuBLIS) uses many of the same ideas to help users build high performance BLAS (basic linear algebra subroutines) libraries for any ISA and microarchitecture simply by editing a config (and possibly writing a micro-kernel).
